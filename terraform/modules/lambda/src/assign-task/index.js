...
const { sendTaskAssignmentEmail } = require('/opt/nodejs/shared/services/email');
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!isAdmin(user)) {
      return error('Only admins can assign tasks', 403);
    }
    ...
    if (newMembers.length === 0) {
      return error('All members are already assigned to this task', 400);
    }
